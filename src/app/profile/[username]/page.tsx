...
export default function ProfilePage() {
  ...
  // Weekly totals (UTC week start)
  const weeklyTotals = useMemo(() => {
    const totals = { study: 0, skill: 0, exercise: 0, all: 0 };

    for (const l of logs) {
      if (new Date(l.occurred_at).getTime() < weekStartMs) continue;
  ...
  const streak = useMemo(() => {
    ...
    for (const l of logs) days.add(dateKeyUTC(new Date(l.occurred_at)));

    const cursor = new Date();
    cursor.setUTCHours(0, 0, 0, 0);

    // forgiving: if no log today (UTC), start from yesterday (UTC)
    if (!days.has(dateKeyUTC(cursor))) cursor.setUTCDate(cursor.getUTCDate() - 1);
        ...
        <>
          ...
          <section className="mt-6 rounded-2xl border p-5">
            ...
            <div className="mt-2 text-xs text-neutral-500">
              Week starts: {weekStart.toLocaleDateString(undefined, { timeZone: "UTC" })} (UTC)
            </div>
          ...
          <div className="mt-4 flex gap-2">
            <button
              ...
              onClick={() =>
                router.push(`/profile/${encodeURIComponent(profile.username)}?tab=posts`)
              }
            >
              Posts
            </button>
            <button
              ...
              onClick={() =>
                router.push(`/profile/${encodeURIComponent(profile.username)}?tab=activity`)
              }
            >
              Activity
            </button>
