...
/** ===== UTC helpers (Option A) ===== */
function startOfWeekMondayUTC(now: Date) {
  // Create a date at UTC midnight (today)
...
export default function ActivityPage() {
  ...
  // Week boundaries computed in UTC (shared for everyone)
  const weekStart = useMemo(() => startOfWeekMondayUTC(new Date()), []);
  ...
  // Logs in THIS week (based on UTC weekStart)
  const weeklyLogs = useMemo(() => {
    return logs.filter((l) => new Date(l.occurred_at).getTime() >= weekStartMs);
  }, [logs, weekStartMs]);
  ...
  const streakByUser = useMemo(() => {
    ...
    for (const uid of Object.keys(activeDays)) {
      ...
      // forgiving streak: if no log today (UTC), start from yesterday (UTC)
      if (!set.has(dateKeyUTC(cursor))) {
  ...
  const createLog = async () => {
    ...
    // User enters local time; we store ISO (UTC) — good for a shared app
    if (occurredAt.trim()) {
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600">
            ...
            <span className="font-semibold text-black">
              {weekStart.toLocaleDateString(undefined, { timeZone: "UTC" })}
            </span>{" "}
            ...
            {totalsByUser.map((u) => (
              <div key={u.user_id} className="rounded-2xl border p-4">
                ...
                <div className="mt-3 text-sm text-neutral-600">
                  ...
                  days <span className="text-xs text-neutral-500">(UTC)</span>
                </div>
              </div>
            ))}
          </div>
                    ...
                    <div>
                      ...
                      >
                        <div className="font-bold">
                          ...
                          <span className="ml-2 font-normal text-neutral-600">@{username}</span>
                        </div>
                      </button>
